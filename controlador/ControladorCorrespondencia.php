...
class GeneradorCorrespondencia{
    ...
    function idBusquedaxNombre($nombre){  
      echo "<script>alert:('bazinga');</script>";
        $sent= "SELECT id_persona AS id, CONCAT(nombre_persona,' ',apellido_persona) as nom FROM corresp_persona WHERE CONCAT(nombre_persona,' ',apellido_persona)= :nombre OR correo_electronico = :nombre";
